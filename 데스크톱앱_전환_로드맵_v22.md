...
- **현재 진행 상태**: ✅ **전체 완료** 🎉
- **최신 앱 버전**: v1.0.68 (GitHub Release)
...
| v22.1 | 2026-02-26 | **KISA 보안 개선** - v1.0.65 릴리즈 (로그인 시도 제한, 비밀번호 확인, console 비활성화, 에러 메시지 정리) |
| v22.2 | 2026-03-04 | **급여현황표 직무대리 직책수당 합산 표시** - v1.0.66 릴리즈 |
| v22.3 | 2026-03-04 | **직원목록/상세 퇴사자 호봉·퇴사일 표시 개선** - v1.0.67 릴리즈 |
| v22.4 | 2026-03-05 | **직원상세 팝업 레이아웃 개선** - v1.0.68 릴리즈 (주민등록번호 줄바꿈 수정, 퇴사자 5열 레이아웃) |
...
## 🤖 AI 도우미 육아휴직/단축근로 정보 추가 (v1.0.60)
...
|------|------|
| 작업일 | 2026년 2월 24일 |
| 목적 | AI 도우미에서 육아휴직 기간, 단축근로 상세정보 질의응답 가능 |
| 원인 | 기존에 `isOnLeave` 여부만 전달, 기간/이력/단축근로 정보 누락 |
...
|--------|------|------|
| 육아휴직 여부 | ✅ '휴직중'만 | ✅ 유지 |
| 육아휴직 기간 | ❌ 누락 | ✅ 시작일, 종료예정일 |
| 육아휴직 이력 | ❌ 누락 | ✅ 과거 이력 (복직, 연속휴직) |
| 임신기 단축근로 | ❌ 누락 | ✅ 기간, 단축시간, 출산예정일 |
| 육아기 단축근로 | ❌ 누락 | ✅ 기간, 주당근무시간, 자녀이름 |

### 수정 파일

| 파일 | 경로 | 변경 내용 |
|------|------|----------|
| AI도우미_인사.js | js/system/ | sanitize에 휴직/단축근로 필드, 통계 섹션, 시스템 프롬프트 규칙 추가 |
...
|------|------|
| 작업일 | 2026년 2월 25일 |
| 목적 | 후보가 1개로 좁혀지면 나머지를 자동 채움 (입력 속도 향상) |
| 방식 | input 이벤트 + setSelectionRange로 하이라이트 표시 |

### 동작 방식

```
"사회" 입력 → 후보 2개 → 드롭다운만 표시 (기존 datalist 동작)
"복지사회" 입력 → 후보 1개 → "정책팀" 자동 채움 + 선택(하이라이트) 상태
→ Tab/Enter: 확정
→ 계속 타이핑: 덮어씀
→ Esc: 자동완성 거부 (입력한 부분만 유지)
→ Backspace/Delete: 정상 삭제 (자동완성 억제)
```

### 한글 입력 처리

- `compositionstart`/`compositionend` 이벤트로 한글 조합 중 자동완성 방지
- 조합 완료 후에만 후보 검색 실행
- Esc 후 플래그 리셋으로 다음 입력 시 자동완성 재활성화

### 수정 파일

| 파일 | 경로 | 변경 내용 |
|------|------|----------|
| 직원등록_인사.js | js/employee/ | `_initInlineAutocomplete()`, `_handleInlineAutocomplete()` 추가 |

---

---

## 🔒 IPC 파일 시스템 경로 보안 강화 (v1.0.62)
...
|------|------|
| 작업일 | 2026년 2월 25일 |
| 목적 | write-file/read-file IPC 핸들러에 시스템 보호 폴더 차단 추가 |
| 배경 | 바이브코딩 보안 점검 후 방어 계층 추가 (안전벨트 개념) |
...
- `BLOCKED_PATHS` 상수 배열로 차단 경로 관리
- `isBlockedPath()` 함수: `path.resolve()` + 소문자 변환으로 정규화 후 비교
- write-file, read-file 핸들러 진입부에서 경로 검증 → 차단 시 에러 반환

### 수정 파일

| 파일 | 경로 | 버전 | 변경 내용 |
|------|------|------|----------|
| main.js | H:\server\hr-system\ | v3.5.0 | `isBlockedPath()`, `BLOCKED_PATHS` 추가, write-file/read-file 검증 |

---

---

## 🔒 sandbox 명시 + CSP 적용 (v1.0.63)
...
|------|------|
| 작업일 | 2026년 2월 25일 |
| 목적 | Electron 보안 Best Practices 적용 (방어 계층 추가) |
| 배경 | Gemini 교차 검증으로 sandbox/CSP 미적용 확인 → 적용 후 테스트 통과 |

### 변경 내용

#### 1. Sandbox 명시 (main.js)

- `webPreferences`에 `sandbox: true` 명시적 추가
- Electron 28에서 암묵적으로 활성화되어 있었으나, 명시적 선언으로 확실히 고정
- preload.js가 `require('electron')`만 사용하므로 충돌 없음 (Gemini 코드 분석으로 검증)

#### 2. CSP 적용 (login.html, 메인_인사.html)

| 지시어 | login.html | 메인_인사.html |
|--------|-----------|---------------|
| default-src | 'self' | 'self' |
| script-src | 'self' 'unsafe-inline' cdn.jsdelivr.net | 'self' 'unsafe-inline' cdn.jsdelivr.net |
| style-src | 'self' 'unsafe-inline' | 'self' 'unsafe-inline' |
| connect-src | 'self' *.supabase.co | 'self' *.supabase.co generativelanguage.googleapis.com blob: |
...
| 파일 | 경로 | 버전 | 변경 내용 |
|------|------|------|----------|
| main.js | H:\server\hr-system\ | v3.6.0 | `sandbox: true` 추가 |
| login.html | H:\server\hr-system\ | - | CSP meta 태그 추가 |
| 메인_인사.html | H:\server\hr-system\ | - | CSP meta 태그 추가 |

### 허용된 외부 도메인 전체 목록

| 도메인 | 용도 | 사용 파일 |
|--------|------|----------|
| cdn.jsdelivr.net | xlsx-js-style, supabase-js | 메인_인사.html, login.html |
| *.supabase.co | 인증, Edge Functions | login.html, 메인_인사.html |
| generativelanguage.googleapis.com | Gemini AI 도우미 | 메인_인사.html |

---

---

## 🤖 AI 도우미 특정 시점 호봉 계산 오류 수정 (v1.0.64)
...
|------|------|
| 작업일 | 2026년 2월 25일 |
| 목적 | "원호성의 2025년 1월 호봉은?" 같은 과거 시점 호봉 질문에 정확한 답변 |
| 원인 | Gemini에 현재호봉만 전달되고 입사호봉/첫승급일이 없어 역산 시 1호봉 오차 발생 |

### 문제

```
질문: "원호성의 2025년 1월 호봉과 2월 호봉은?"
오답: 1월 19호봉, 2월 20호봉
정답: 1월 18호봉, 2월 19호봉
```
...
| 전달된 데이터 | 누락된 데이터 |
|-------------|-------------|
| 현재호봉 (20) | ❌ 입사호봉 (startRank) |
| 매년승급일 (2월 1일) | ❌ 첫승급일 (firstUpgradeDate) |

Gemini가 기준점 없이 현재호봉에서 역산하여 1호봉 차이 발생

### 수정 내용

| 위치 | 변경 |
|------|------|
| `_sanitizeEmployeeForAI()` | `입사호봉`, `첫승급일` 필드 추가 |
| 시스템 프롬프트 | `[특정 시점 호봉 계산 규칙]` 섹션 추가 (정방향 계산 예시, 역산 금지) |
| 컨텍스트 출력 | 각 직원에 `입사호봉: N`, `첫승급일: YYYY-MM-DD` 표시 |

### 수정 파일

| 파일 | 경로 | 변경 내용 |
|------|------|----------|
| AI도우미_인사.js | js/system/ | 입사호봉/첫승급일 전달, 시스템 프롬프트 호봉 계산 규칙 추가 |
...
|------|------|
| 작업일 | 2026년 2월 26일 |
| 목적 | KISA 소프트웨어 개발보안 가이드 기준 보안 취약점 개선 |
| 적합률 | 81% → 87% 향상 |
...
| # | 항목 | 내용 |
|---|------|------|
| 1 | 로그인 시도 제한 | 5회 연속 실패 시 1분 잠금, 카운트다운 타이머 표시 |
| 2 | 회원가입 비밀번호 확인 | 비밀번호 재입력 필드 추가, 불일치 시 가입 차단 |
| 3 | console 출력 비활성화 | 프로덕션 환경에서 console.log/info/debug 차단 (error, warn 유지) |
| 4 | 에러 메시지 정리 | 사용자 노출 alert에서 error.message 기술 정보 제거 |

### 수정 파일

| 파일 | 경로 | 변경 내용 |
|------|------|----------|
| login.html | / | 로그인 시도 제한 (v1.3.0), 비밀번호 확인 (v1.3.1), 에러 메시지 정리 |
| main.js | / | 프로덕션 console 비활성화 (v3.7.0) |
| 메인_인사.html | / | 렌더러 프로세스 console 비활성화 |
| 경력편집_인사.js | js/career/ | error.message 노출 제거 (2개소) |
| 직원등록_인사.js | js/employee/ | error.message 노출 제거 (1개소) |
| 통계분석_인사.js | js/reports/ | error.message 노출 제거 (1개소) |

### 릴리즈 이력

| 버전 | 날짜 | 내용 |
|------|------|------|
| v1.0.65 | 2026-02-26 | KISA 보안 개선 (로그인 제한, 비밀번호 확인, console 비활성화, 에러 메시지 정리) |
| v1.0.66 | 2026-03-04 | 급여현황표 직무대리 직책수당 합산 표시 (테이블/엑셀/인쇄) |
| v1.0.67 | 2026-03-04 | 직원목록/상세 퇴사자 호봉·퇴사일 표시 개선 (퇴사일 기준 호봉, 퇴사일 컬럼 추가) |
| v1.0.68 | 2026-03-05 | 직원상세 팝업 레이아웃 개선 (주민등록번호 줄바꿈 수정, 퇴사자 5열 레이아웃) |
