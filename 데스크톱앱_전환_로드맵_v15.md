...
- **현재 진행 상태**: ✅ **전체 완료** 🎉
- **최신 앱 버전**: v1.0.49 (GitHub Release)
...
| 랜딩페이지 | C:\sw-hrm-landing | Vercel 자동 배포 |
| 관리자 페이지 | C:\sw-hrm-admin | Vercel 자동 배포 |
...
| 파일 | 버전 | 경로 | 변경 내용 |
|------|------|------|----------|
| main.js | v3.3.0 | C:\hr-system\ | `focus-window` IPC 핸들러 (blur/focus 트릭) |
| preload.js | v2.2.0 | C:\hr-system\ | `focusWindow` API 노출 |
...
**수정 파일:** `C:\sw-hrm-admin\index.html`
...
|------|------|
| 작업일 | 2026년 2월 9일 |
| 대상 | C:\sw-hrm-landing\index.html |
...
|------|------|
| 함수명 | org-count |
| 경로 | C:\hr-system-server\supabase\functions\org-count\index.ts |
...
| 파일 | 경로 | 수정 내용 |
|------|------|----------|
| 메인_인사.html | C:\hr-system\ | title, 사이드바 헤더, 대시보드 인사말, 프로그램 정보 |
...
- 해결: `Remove-Item "C:\hr-system\dist\*" -Recurse -Force` 후 재빌드
