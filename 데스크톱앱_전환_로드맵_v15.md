...
- **현재 진행 상태**: ✅ **전체 완료** 🎉
- **최신 앱 버전**: v1.0.28 (GitHub Release)
- **최신 릴리즈**: https://github.com/mangfeel/hr-system/releases
- **저장소 상태**: Public (향후 Private 전환 예정 — [향후 과제](#-향후-과제--github-저장소-private-전환) 참고)
...
| 저장소 | URL | 상태 | 자동 업데이트 |
|--------|-----|------|-------------|
| HR 앱 | https://github.com/mangfeel/hr-system | **Public** | ✅ 토큰 없이도 동작 (토큰도 포함됨) |
| 시간외 앱 | https://github.com/mangfeel/overtime-system | **Private** | ✅ 토큰으로 인증 |
...
- 토큰은 추가되어 있으므로 향후 Private 전환 시 자동 업데이트 즉시 동작
- **v1.0.28이 모든 사용자에게 배포된 후에만 Private 전환 가능** (닭과 달걀 문제)
- Private 전환 전 배포된 구버전(토큰 없음)은 자동 업데이트 불가 → 수동 설치 필요
...
| 선행 조건 | 모든 사용자가 v1.0.28 이상으로 업데이트 완료 |
| 우선순위 | 상용화 시점에 진행 |
...
| 구버전(v1.0.27 이하) 자동 업데이트 불가 | 모든 사용자 v1.0.28+ 업데이트 확인 필수 |
